...
inp = out.input
eig = lp.get_eigenval(file_eigenv, inp)

lvec_rec = out.get_lattice()[1]
ef       = out.energy['E_fermi']
...
if out_h:
    with open('eigenval.h.dat','w') as f:
        f.write('# Type: H\n')
        if ismetal:
            f.write('# %10s %10s %10s\n'%('ISPIN','NBANDS','NKPT'))
        ...
        for iSpin in range(eig.ispin):
            ...
            for iKpt in range(eig.nkpts):
                ...
                for i in range(3): 
                    f.write(' %10.6f'%(eig.kpts[iKpt,i]))
                for iBand in range(eig.nbands):
                    f.write(' %12.6f'%(eig.eigens[iKpt,iBand,iSpin]))
                if (iKpt == 0) or (lenkpt[iKpt] == lenkpt[iKpt-1]): 
                    f.write(' # HSP')
                f.write('\n')
            f.write('#\n')

if out_v:
    with open('eigenval.v.dat','w') as f:
        f.write('# Type: V\n')
        if ismetal:
            f.write('# %10s %10s %10s\n'%('ISPIN','NBANDS','NKPT'))
            f.write('# %10i %10i %10i\n'%(eig.ispin, eig.nbands, eig.nkpts))
