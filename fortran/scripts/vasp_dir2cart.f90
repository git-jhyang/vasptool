...
if ((bc200(i:i) .eq. 'C') .or. (bc200(i:i) .eq. 'c')) then
   write (*,*) " !!! WRANRING : Your 'POSCAR' is already 'Cartesian' !!!"
   iscart = .true.
else if ((bc200(i:i) .eq. 'D') .or. (bc200(i:i) .eq. 'd')) then
   iscart = .false.
endif
write (fo, '(A)') "Cartesian"
...
subroutine dir_to_cart(lvec, coord)
implicit none
...
do i = 1, 3
   do while ( coord(i) .lt. 0 )
      coord(i) = coord(i) + 1.0_fp
   enddo
   do while ( coord(i) .ge. 1.0_fp )
      coord(i) = coord(i) - 1.0_fp
   enddo
enddo
