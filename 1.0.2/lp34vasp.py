...
def get_str(str, file, n=-1):
   ...
   for line in f:
      if str in line:
         ...
         if (type(n) == int) and (np.shape(lines)[0]-1 == n): break
   f.close()
   if np.shape(lines)[0] == 0: return None
   if type(n) is int:
      if n > np.shape(lines)[0]-1 or n < 0: return lines[-1]
      return lines[n]
   elif n == 'all' or n[0] == 'a':
      return lines
   else:
      call_error('input error - get_str(n)',n)
...
class get_outcar(object):
   ...
   .energy   : return calculated VASP total energy in PyDic
