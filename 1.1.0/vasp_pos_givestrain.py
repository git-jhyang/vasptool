...
for i, arg in enumerate(sys.argv):
    ...
    for str in arg[1:]:
        if str == 'h': call_help()
            ...
            if (len(sys.argv) < i): 
                call_help('input arguments','strain is missing')
            if (not lp.assert_num(sys.argv[i+1])): 
                call_help('input arguments -i','not a number '+sys.argv[i+1])
            axis[0] = True
            ...
            if (len(sys.argv) < i): 
                call_help('input arguments','strain is missing')
            if (not lp.assert_num(sys.argv[i+1])): 
                call_help('input arguments -x','not a number '+sys.argv[i+1])
            axis[1] = True
            ...
            if (len(sys.argv) < i): 
                call_help('input arguments','strain is missing')
            if (not lp.assert_num(sys.argv[i+1])): 
                call_help('input arguments -y','not a number '+sys.argv[i+1])
            axis[2] = True
            ...
            if (len(sys.argv) < i): 
                call_help('input arguments','strain is missing')
            if (not lp.assert_num(sys.argv[i+1])): 
                call_help('input arguments -z','not a number '+sys.argv[i+1])
            axis[3] = True
            strain[3] = np.float(sys.argv[i+1])
        elif str == 'n':
            if (len(sys.argv) < i): 
                call_help('input arguments','variable is missing')
            if (not lp.assert_num(sys.argv[i+1])): 
                call_help('input arguments -n','not a number '+sys.argv[i+1])
...
if pos.iscart: 
    lp.cart_to_dir(pos.lvec, pos_re.coord)
if pos.iscart: 
    pos_re.iscart = False
...
for img in range(1,images+1):
    for i in range(3):
        if axis[i+1]: 
            ...
            pos_re.lvec[:][i] = pos.lvec[:][i]*ratio
    if axis[0]:
        ...
        for i in range(3):
            if axis[i+1]: 
                str += '_%s_%.3f'%(xyz[i], 1e0+strain[i+1]*img/images)
    pos_re.de   s = str+'\n'
    pos_re.f_name = 'POSCAR_%s'%(str[7:])
