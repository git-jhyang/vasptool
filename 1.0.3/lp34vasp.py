...
def assert_file(f, str='', isopen=True):
   ...
   if os.path.isfile(f):
      ...
      if isopen: call_error(f+' is missing',str)
      return False
...
def get_str(str, file, n=-1):
   ...
   f.close()
   comm = 'grep \'%s\' %s '%(str, file)
   nLine = int(os.popen(comm+'| wc -l').read())
   if (nLine == 0): call_error('read error - get_str','%s is missing in file %s'%(str,file))
   if (type(n) == int):
      iLine = n+1
      ...
      if ((n != 'all') and (n[0] != 'a')): call_error('input error - get_str(n)',n)
...
class get_outcar(object):
   ...
   def __init__(self, filename):
      ...
      assert_file(self.filename,'get_outcar', isopen=False)
      ...
      self.is_band = False
      self.is_stat = False
      if get_str('interpolating k-points', self.filename, n=0) is not None: self.is_band = True
   ...
   def get_energy(self):
      enes = {}
      enes['E_fermi']  = np.float_(chk_len(get_str('E-fermi', self.filename, n=-1), 2))
      enes['E_free']   = np.float_(chk_len(get_str('free  energy', self.filename, n=-1), 4))
      line = get_str('energy  without', self.filename, n=-1)
...
# get information
      ...
      if not lp.assert_dat(inp['nelect'], eig.nelect):
         call_error('input error - NELECT','%i %i'%(inp['nelect']), eig.nelect)
      if not lp.assert_dat(inp['ispin'], eig.ispin):
         call_error('input error - ISPIN','%i %i'%(inp['ispin']), eig.ispin)
...
class get_poscar(object):
   ...
   get_poscar(filename)
   ------------------------------------------------------------------
   read VASP POCSCAR 'filename' and return dataset 'dat'.
   ------------------------------------------------------------------
   - output -
...
class write_poscar(object):
   """
   write_poscar(filename, dat, islong=True)
   ------------------------------------------------------------------
   ...
   """
   def __init__(self, filename, dat, islong=True):
...
class get_xsf(object):
   """
   get_xsf(filename, dat)
   ------------------------------------------------------------------
   read XCrysDen XSF 'filename' and return dataset 'dat'
   ------------------------------------------------------------------
   - output -
   .filename : filename of XSF
   ...
   def __init__(self, filename):
      self.filename = filename
      f = assert_file(self.filename, 'get_xsf', isopen=True)
      self.des = f.readline()
      self.iscart = True
      cnt = 0
...
# read coord and vec
      for i in range(self.natoms):
         ...
         if getvec: vector[i][j] = np.float_(buf[j+4]) for j in range(3)
